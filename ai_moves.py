# ai player returns generally moves that guarantees the fastest victory
import pandas as pd
import numpy as np
import random

df = pd.read_csv("outcomes.csv",engine='pyarrow').astype('int8', errors='ignore')

#divide into specific outcomes
...
#used to return move sequence in another module
def sequence():
...
#saves the game sequence if it hasn't already happened and transposes the game board 8-ways, since there 8 possible mirror images of it
...
def ai_move(ai_first,board):
    ...
    #print statements are mostly used for debugging
    if actual_move == 0:
        # print('move0')
        if ai_first == True:
        ...
        if ai_first == True:
            ...
            if win_first.empty == False and not ((set(legal_moves)-set(win_first.iloc[:,actual_move].values.flatten()))==set(legal_moves)):
                # print('win first')
                # print(actual_move)
                win_first=sort_df_by_zeros(win_first)
            ...
            elif lost_notfirst.empty == False and not(set(legal_moves)-set(lost_notfirst.iloc[:,actual_move].values.flatten())==set(legal_moves)):
                ...
                return 11 #just to make sure the output is an int
            ...
            if win_notfirst.empty == False and not(set(legal_moves)-set(win_notfirst.iloc[:,actual_move].values.flatten())==set(legal_moves)):
            ...
            elif lost_first.empty == False and not(set(legal_moves)-set(lost_first.iloc[:,actual_move].values.flatten())==set(legal_moves)):
